...
> **Adapted for this repository.** The published skill writes to a Cowork
> desktop sandbox (`/sessions/.../mnt/outputs/`). Paths here point at
> `docs/audits/` for reports and at `.claude/skills/docx/scripts/office/` for
> the validate and PDF-conversion helpers, which is where they live in this
> project. Nothing else was changed.
>
> **It audits a LIVE URL.** It cannot audit an undeployed site — deploy first,
> then point it at the real address.
...
Packer.toBuffer(doc).then(buffer => {
  fs.writeFileSync('docs/audits/seo-audit-[domain]-[date].docx', buffer);
...
```bash
python .claude/skills/docx/scripts/office/validate.py docs/audits/seo-audit-[domain]-[date].docx
```
...
```bash
python .claude/skills/docx/scripts/office/soffice.py --headless --convert-to pdf docs/audits/seo-audit-[domain]-[date].docx --outdir docs/audits/
```

### Deliver to the user

Deliver both files with the SendUserFile tool, then give their repository paths:

```
Your audit report is ready:
[Download Word Doc](computer://docs/audits/seo-audit-[domain]-[date].docx)
[Download PDF](computer://docs/audits/seo-audit-[domain]-[date].pdf)
```
