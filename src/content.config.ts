...
const machines = defineCollection({
  ...
  schema: ({ image }) =>
    z.object({
      ...
      /* Normally exactly two. This preview ships a single machine, so an
         empty list is allowed and the related section simply does not render. */
      related: z.array(z.string()).max(2),
